...
def _coerce_duration(raw: Any, default: str = "") -> str:
    """Normalise whatever the LLM returned for a duration field.

    Defaults to an empty string so that unset durations remain unset rather
    than being filled in with an arbitrary "30 minutes" stub. Callers can
    decide how to handle the empty case (drop step, log warning, etc.).
    """
    if raw is None:
        return default
...
def _normalize_protocol_steps(raw_steps: Any, prompt: str) -> list[dict[str, Any]]:
    ...
    for idx, entry in enumerate(raw_steps, start=1):
        ...
        if action == details:
            details = ""
        # Drop steps the LLM left empty rather than padding with "Step N".
        if not action:
            continue
        normalised.append(
            {
...
async def literature_scout(
    ...
    *,
    ...
    # `use_mock` no longer returns a hardcoded CRP reference. The literature
    # scout always queries Tavily + OpenAI; failures are reported via the
    # rescue path below instead of being masked with fake citations.
...
async def protocol_designer(
    ...
    *,
    ...
    # Note: `use_mock` no longer triggers a hardcoded CRP-style stub. Every
    # protocol is generated by the LLM from the actual hypothesis. If the
    # caller has no OpenAI key we fall through to the rescue path below
    # which surfaces the error in `_warning` instead of inventing data.
    protocol_schema = (
        "Return JSON with keys: steps (list), total_duration (string), "
        "controls (list of strings). Each step MUST include step_number "
        "(int), action (short imperative string), duration (human-readable, "
        "e.g. '45 min', '2 h', '24 h'), and details (one sentence with "
        "concrete reagents, concentrations, temperatures, instrument "
        "settings — derived directly from the hypothesis). Produce 6 to 12 "
        "concrete, hypothesis-specific steps. Every step, control, and "
        "duration MUST be derived from the hypothesis — do NOT emit "
        "generic placeholder language like 'prepare reagents' or 'run "
        "assay'."
    )
    ...
    *,
    ...
    for idx, phase in enumerate(raw_phases, start=1):
        ...
        try:
            ...
            start_day = idx
        # Skip phases the LLM left empty instead of inserting "Phase N" /
        # "1 day" placeholders.
        name_str = str(name or "").strip()
        duration_str = str(duration or "").strip()
        if not name_str or not duration_str or not tasks:
            continue
    ...
    out: dict[str, Any] = {
        ...
        "total_duration": str(total_duration_raw).strip(),
    }
    if warning:
        out["_warning"] = warning
    if not normalised_phases:
        out["_warning"] = (warning or "") + " | LLM produced no usable phases"
    return out
...
async def _merge_budget_with_prices(
    ...
    system = (
        ...
        "discrepancies in the form '<item>: stated <X> EUR, supplier page "
        "shows <Y> EUR (<domain>)'. If all prices check out, set notes to 'All "
...
async def _merge_timeline_with_benchmarks(
    ...
    for idx, phase in enumerate(phases_raw):
        if not isinstance(phase, dict):
            continue
        # Fall back to the LLM-generated draft phase if the merger LLM
        # omitted a field — never to a hardcoded "Phase N" / "1 day".
        fallback = draft_phases[idx] if idx < len(draft_phases) and isinstance(draft_phases[idx], dict) else {}
        name = str(phase.get("phase") or fallback.get("phase") or "").strip()
        duration = str(phase.get("duration") or fallback.get("duration") or "").strip()
        ...
        if not name or not duration or not tasks:
            continue
        entry = {
            "phase": name,
            "duration": duration,
